...
import { getCategories, getProductDetails, saveProduct } from '@/app/actions/products'
import { 
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (product) {
        ...
        const imgs = product.product_images?.map((img: any) => img.image_url) || []
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 flex flex-col pb-28 relative overflow-hidden">
      ...
      <div className="px-4 py-4 z-10 flex items-center gap-3 border-b border-slate-900 bg-slate-950/60 backdrop-blur-md sticky top-0">
        ...
        <span className="font-extrabold text-sm text-slate-300">Edit Product</span>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="pt-2">
            ...
            >
              ...
              {submitting ? 'Saving Item...' : 'Save Changes'}
            </button>
          </div>
        </form>
      </div>
